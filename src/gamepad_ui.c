...
void draw_ui(void) {
  ...
  DrawText(TextFormat("Server Status: RUNNING on port %d", PORT), 40, 85, 20,
           WHITE);
  DrawText(TextFormat("Connected Clients: %d / %d", server_state.client_count,
                      MAX_CLIENTS),
           40, 115, 20, YELLOW);
  DrawText(
  ...
  for (int i = 0; i < MAX_CLIENTS; i++) {
    if (server_state.clients[i].active) {
      ...
      DrawText(TextFormat("Client #%d", i + 1), 40, y_offset + 10, 18, WHITE);
  ...
  if (server_state.client_count == 0) {
    DrawText("No clients connected. Waiting for connections...", 40, y_offset,
             18, DARKGRAY);
  }
