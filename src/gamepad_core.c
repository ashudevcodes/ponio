...
int init_virtual_gamepad(void)
{
	...
	struct input_absinfo absinfo = {
		.value = 0, .minimum = -32768, .maximum = 32767, .fuzz = 0, .flat = 0, .resolution = 0
	};
...
void process_command(const char *command)
{
	...
	if (strncmp(cmd, "BTN_", 4) == 0) {
		...
		if (strcmp(cmd, "BTN_A") == 0) {
			libevdev_uinput_write_event(server_state.uinput_dev, EV_KEY, BTN_A, value);
		} else if (strcmp(cmd, "BTN_B") == 0) {
			libevdev_uinput_write_event(server_state.uinput_dev, EV_KEY, BTN_B, value);
		} else if (strcmp(cmd, "BTN_X") == 0) {
			libevdev_uinput_write_event(server_state.uinput_dev, EV_KEY, BTN_X, value);
		} else if (strcmp(cmd, "BTN_Y") == 0) {
			libevdev_uinput_write_event(server_state.uinput_dev, EV_KEY, BTN_Y, value);
		} else if (strcmp(cmd, "BTN_L1") == 0) {
			libevdev_uinput_write_event(server_state.uinput_dev, EV_KEY, BTN_TL, value);
		} else if (strcmp(cmd, "BTN_R1") == 0) {
			libevdev_uinput_write_event(server_state.uinput_dev, EV_KEY, BTN_TR, value);
		} else if (strcmp(cmd, "BTN_L2") == 0) {
			libevdev_uinput_write_event(server_state.uinput_dev, EV_KEY, BTN_TL2, value);
		} else if (strcmp(cmd, "BTN_R2") == 0) {
			libevdev_uinput_write_event(server_state.uinput_dev, EV_KEY, BTN_TR2, value);
		} else if (strcmp(cmd, "BTN_START") == 0) {
			libevdev_uinput_write_event(server_state.uinput_dev, EV_KEY, BTN_START, value);
		} else if (strcmp(cmd, "BTN_SELECT") == 0) {
			libevdev_uinput_write_event(server_state.uinput_dev, EV_KEY, BTN_SELECT, value);
		}

		libevdev_uinput_write_event(server_state.uinput_dev, EV_SYN, SYN_REPORT, 0);
	} else if (strncmp(cmd, "DPAD_", 5) == 0) {
		value = atoi(value_str);

		if (strcmp(cmd, "DPAD_UP") == 0) {
			libevdev_uinput_write_event(server_state.uinput_dev, EV_KEY, BTN_DPAD_UP, value);
		} else if (strcmp(cmd, "DPAD_DOWN") == 0) {
			libevdev_uinput_write_event(server_state.uinput_dev, EV_KEY, BTN_DPAD_DOWN, value);
		} else if (strcmp(cmd, "DPAD_LEFT") == 0) {
			libevdev_uinput_write_event(server_state.uinput_dev, EV_KEY, BTN_DPAD_LEFT, value);
		} else if (strcmp(cmd, "DPAD_RIGHT") == 0) {
			libevdev_uinput_write_event(server_state.uinput_dev, EV_KEY, BTN_DPAD_RIGHT, value);
		}
		...
		if (sscanf(value_str, "%f,%f", &x, &y) == 2) {
			...
			if (strcmp(cmd, "LJOY") == 0) {
				libevdev_uinput_write_event(server_state.uinput_dev, EV_ABS, ABS_X, axis_x);
				libevdev_uinput_write_event(server_state.uinput_dev, EV_ABS, ABS_Y, axis_y);
			} else {
				libevdev_uinput_write_event(server_state.uinput_dev, EV_ABS, ABS_RX, axis_x);
				libevdev_uinput_write_event(server_state.uinput_dev, EV_ABS, ABS_RY, axis_y);
			}
...
void *handle_client(void *arg)
{
	...
	while (server_state.server_running && client->active) {
		...
		while (line != NULL) {
			if (strlen(line) > 0) {
				pthread_mutex_lock(&server_state.lock);
				strncpy(client->last_command, line, sizeof(client->last_command) - 1);
				client->commands_received++;
				pthread_mutex_unlock(&server_state.lock);

				if (strncmp(line, "DISCONNECT", 10) != 0 && strncmp(line, "CONNECT", 7) != 0) {
...
void *server_thread(void *arg)
{
	...
	int opt = 1;
	setsockopt(server_state.server_socket, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt));
	...
	if (bind(server_state.server_socket, (struct sockaddr *)&server_addr, sizeof(server_addr)) <
	    0) {
		perror("Bind failed");
	...
	while (server_state.server_running) {
		int client_socket =
		    accept(server_state.server_socket, (struct sockaddr *)&client_addr, &client_len);
