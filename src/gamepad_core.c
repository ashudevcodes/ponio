...
void process_command(const char *command) {
  ...
  if (strncmp(cmd, "BTN_", 4) == 0) {
    ...
    if (strcmp(cmd, "BTN_A") == 0) {
      ...
      libevdev_uinput_write_event(server_state.uinput_dev, EV_KEY, BTN_TL2,
                                  value);
    } else if (strcmp(cmd, "BTN_R2") == 0) {
      libevdev_uinput_write_event(server_state.uinput_dev, EV_KEY, BTN_TR2,
                                  value);
    } else if (strcmp(cmd, "BTN_START") == 0) {
      libevdev_uinput_write_event(server_state.uinput_dev, EV_KEY, BTN_START,
                                  value);
    } else if (strcmp(cmd, "BTN_SELECT") == 0) {
      libevdev_uinput_write_event(server_state.uinput_dev, EV_KEY, BTN_SELECT,
                                  value);
    }
  ...
  // D-Pad commands
    ...
    if (strcmp(cmd, "DPAD_UP") == 0) {
      libevdev_uinput_write_event(server_state.uinput_dev, EV_KEY, BTN_DPAD_UP,
                                  value);
  ...
  // Joystick commands
    ...
    if (sscanf(value_str, "%f,%f", &x, &y) == 2) {
      ...
      if (strcmp(cmd, "LJOY") == 0) {
        libevdev_uinput_write_event(server_state.uinput_dev, EV_ABS, ABS_X,
                                    axis_x);
        libevdev_uinput_write_event(server_state.uinput_dev, EV_ABS, ABS_Y,
                                    axis_y);
      } else {
        libevdev_uinput_write_event(server_state.uinput_dev, EV_ABS, ABS_RX,
                                    axis_x);
        libevdev_uinput_write_event(server_state.uinput_dev, EV_ABS, ABS_RY,
                                    axis_y);
      }
...
//   server_state.server_socket = socket(AF_INET, SOCK_DGRAM, 0);
//
...
//     int bytes = recvfrom(server_state.server_socket, buffer, sizeof(buffer) -
//     1,
...
void *server_thread(void *arg) {
  ...
  // Accept connections
  while (server_state.server_running) {
    int client_socket = accept(server_state.server_socket,
    ...
    if (slot >= 0) {
      ...
      inet_ntop(AF_INET, &client_addr.sin_addr, server_state.clients[slot].ip,
                INET_ADDRSTRLEN);
      server_state.client_count++;
